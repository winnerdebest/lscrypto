  ...
  return (
    <div className="w-full">
      <div className="mb-10 text-center lg:text-left">
        <h1 className="text-3xl font-heading font-bold mb-3">Create Account</h1>
        <p className="text-[var(--text-secondary)]">Start your institutional crypto journey today.</p>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <input {...register('first_name')} className="input bg-[#0A0D18] focus:bg-[#080B14] transition-colors" placeholder="Alexander" />
            {errors.first_name && <p className="text-[var(--color-danger)] text-xs mt-1.5">{errors.first_name.message}</p>}
          </div>
          <div>
            <label className="block text-sm font-medium text-[var(--text-secondary)] mb-2">Last Name</label>
            <input {...register('last_name')} className="input bg-[#0A0D18] focus:bg-[#080B14] transition-colors" placeholder="Thorne" />
            {errors.last_name && <p className="text-[var(--color-danger)] text-xs mt-1.5">{errors.last_name.message}</p>}
          </div>
        </div>

        <div>
          <label className="block text-sm font-medium text-[var(--text-secondary)] mb-2">Email Address</label>
          <input type="email" {...register('email')} className="input bg-[#0A0D18] focus:bg-[#080B14] transition-colors" placeholder="alexander@example.com" />
          {errors.email && <p className="text-[var(--color-danger)] text-xs mt-1.5">{errors.email.message}</p>}
        </div>

        <div>
          <label className="block text-sm font-medium text-[var(--text-secondary)] mb-2">Password</label>
          <input type="password" {...register('password')} className="input bg-[#0A0D18] focus:bg-[#080B14] transition-colors" placeholder="••••••••" />
        ...
        <button type="submit" disabled={isLoading} className="btn-primary w-full mt-8 py-4 text-base group hover:shadow-[0_0_30px_var(--accent-primary-glow)]">
          {isLoading ? 'Provisioning vault...' : 'Create Vault Account'}
        </button>
      </form>

      <div className="my-8 flex items-center justify-center space-x-4">
        <div className="h-[1px] flex-1 bg-[var(--border-glass)]"></div>
        <span className="text-xs text-[var(--text-muted)] font-bold tracking-widest uppercase">OR SIGN UP WITH</span>
        <div className="h-[1px] flex-1 bg-[var(--border-glass)]"></div>
      </div>
